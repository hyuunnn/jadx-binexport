...
3c. **Progress + cancel (both GUI actions).** A big app takes minutes, so the GUI shows an `ExportProgressDialog` (Swing bar + Cancel), like the IDA BinExport wait box / Ghidra `TaskMonitor`. The plumbing is a headless `ExportProgress` interface (`stage(label,total)` / `update(done,total)` / `cancelled()`), threaded through `Exporter.run`/`runToFile` and `BinDiffRunner.diff`; the CLI/library path passes `ExportProgress.NONE`, keeping `Exporter` free of any Swing dependency and headless-testable. The exporter reports two stages — "Decompiling classes" (the forceProcess loop, the dominant cost) and "Building flow graphs" — with UI updates **throttled** (every 64 classes / 256 methods) so 250k methods don't post 250k EDT events, and polls `cancelled()` in both loops, throwing `Exporter.CancelledException`. **Cancel is honored across the whole diff flow, not just the export:** `BinDiffRunner.runProcess` polls `cancelled()` in 200ms slices while bindiff runs and `destroyForcibly()`-kills the process on cancel (throwing `CancelledException`), and `promptAndDiff` re-checks after the result load. Cancel is clean because the export file is written only *after* all building, so an aborted run leaves nothing behind (the diff path's `finally` still deletes the temp work dir). `runLogged` and both GUI actions catch `CancelledException` distinctly from a real failure (info log, no error dialog). The dialog is thread-safe by construction: all Swing access is marshalled to the EDT via `invokeLater` (FIFO-ordered after the constructor's build post); `cancelled()` reads a `volatile` the Cancel button sets on the EDT.
...
4a. Minor efficiency cleanups deferred (measured negligible): the diff flow opens the SQLite DB twice (`loadMatches` + `loadHeader` each call `open`) — one connection would do; `Exporter.emitInstruction` allocates a fresh `LinkedHashSet` per instruction for callees even though most instructions have none — lazy/reused set would cut ~90k throwaway allocations.
