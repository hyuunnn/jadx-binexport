...
### jadx에서 결과 보고 함수로 이동 (더블클릭 네비게이션)

IDA BinDiff 플러그인처럼, 매칭 결과를 jadx-gui 안에서 훑어보고 해당 함수로 바로
점프할 수 있다. jadx-gui에 앱을 연 상태에서:

**Plugins → Open BinDiff results (.BinDiff)…** → 위에서 만든 `.BinDiff` 선택.

열린 앱에 속한 매칭 함수들이 테이블로 뜨고, 유사도 순(바뀐 함수부터)으로 정렬되어
빨강→초록으로 색칠된다. **행을 더블클릭(또는 Enter)하면 그 메서드가 jadx에서 열린다.**
필터 상자로 목록을 좁힐 수 있다. 매칭은 이름으로 연결되므로(익스포터가 각 메서드의
전체 시그니처를 `mangled_name`으로 기록) 주소가 합성값이어도 네비게이션이 동작한다 —
단, `.BinDiff`를 같은 앱·같은 jadx 버전으로 연 jadx 세션에서 로드해야 한다.
